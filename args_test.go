...
func TestRefsArgs(t *testing.T) {
	...
	t.Run("exported field set is exactly the documented one", func(t *testing.T) {
		...
		got := exportedFieldNames(reflect.TypeOf(RefsArgs{}))
		assert.Equal(t, want, got)
	})

	t.Run("field types are as documented", func(t *testing.T) {
		typ := reflect.TypeOf(RefsArgs{})
	...
	t.Run("no methods on the type", func(t *testing.T) {
		// RefsArgs is a plain data carrier — no methods on the value
		// or pointer receiver.
		assert.Equal(t, 0, reflect.TypeOf(RefsArgs{}).NumMethod())
		assert.Equal(t, 0, reflect.PointerTo(reflect.TypeOf(RefsArgs{})).NumMethod())
	})
}

func TestObjectInfoArgs(t *testing.T) {
	t.Run("zero value", func(t *testing.T) {
		var a ObjectInfoArgs
		assert.False(t, a.Size)
	})

	t.Run("populated values survive round trip", func(t *testing.T) {
		a := ObjectInfoArgs{Size: true}
		assert.True(t, a.Size)
	})

	t.Run("exported field set is exactly the documented one", func(t *testing.T) {
		want := []string{"Size"}
		got := exportedFieldNames(reflect.TypeOf(ObjectInfoArgs{}))
		assert.Equal(t, want, got)
	})

	t.Run("field types are as documented", func(t *testing.T) {
		typ := reflect.TypeOf(ObjectInfoArgs{})
		assertFieldType(t, typ, "Size", reflect.TypeOf(false))
	})

	t.Run("no methods on the type", func(t *testing.T) {
		assert.Equal(t, 0, reflect.TypeOf(ObjectInfoArgs{}).NumMethod())
		assert.Equal(t, 0, reflect.PointerTo(reflect.TypeOf(ObjectInfoArgs{})).NumMethod())
	})
}
