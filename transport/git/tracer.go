...
// t into a freshly constructed [pktline.Reader] reading bytes flowing
// inbound (server-to-client) over the TCP connection. The redacted URL
// is attached so emitted [trace.PacketEvent] values carry the git://
// URL the [Conn] was opened against.
//
...
// `transport/ssh/tracer.go` so a tracer wired through
...
// [inboundReaderOpts] for the writing side of the TCP connection — the
