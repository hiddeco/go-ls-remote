...
// RefsArgs collects the per-command arguments for an `ls-refs`
// invocation — the inputs a caller passes to [Session.Refs],
// [Session.ListRefs], and the corresponding top-level helpers.
//
// RefsArgs is a plain data carrier with no methods. The zero value is
// valid and asks the server for every ref it would normally advertise,
// without peeled object ids, symref targets, or an unborn `HEAD`.
//
// # Field semantics
//
//   - [RefsArgs.Prefixes] restricts the returned refs to those whose
...
//     side. Either way, an empty or nil [RefsArgs.Prefixes] means "do
//     not filter".
//   - [RefsArgs.Peel] asks the server to include the peeled object id
//     for annotated tags. On v2 it maps to the `peel` argument to
...
//   - [RefsArgs.Symrefs] asks the server to disclose symref targets
...
//     effect there.
//   - [RefsArgs.Unborn] asks a v2 server to advertise an unborn
