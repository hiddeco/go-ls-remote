...
// loop has already consumed the `command=ls-refs` line and the
// trailing delim of the capability section; the handler reads the
// command-args section up to the terminating flush
// (`serve.c::process_request` lines 323-329 and `ls-refs.c::ls_refs`)
// and writes the ref list followed by a flush.
//
...
// replaces the body with the full ref enumeration; the signature is
// stable so callers and the dispatcher do not change.
func handleLSRefs(r *pktline.Reader, w *pktline.Writer,
	store *objstore.Store, opts Options) error {
	_ = store
	_ = opts
	if err := drainArgs(r); err != nil {
		return fmt.Errorf("server: ls-refs: drain args: %w", err)
	}
...
// `ls-refs.c::ls_refs`, `object-info.c::object_info`) consume args
// one at a time and stop on `PACKET_READ_FLUSH`; the stub mirrors that
// loop without interpreting any argument.
//
