...
//
// Wire-layer sentinels are joined onto their public counterparts via
// [bridgeWireSentinel] so callers match `errors.Is(err, ErrX)`
// without walking the wire layer. Mid-session refusals (an unknown
// command, an `ERR` pkt-line mid-stream, an unrecognised `ls-refs`
// argument) all flow through this hook.
